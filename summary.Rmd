...
diversity_file <-
  "D:/c01 Data/us_map_ethnicities.csv"
...
entropy_index <- function(...) {
  ...
  percentage_vector <- d[d != 0]
  
  R = length(percentage_vector)
  
  e <- numeric(R) # Create vector of zeros
  
  for (i in 1:R) {
    e[i] = percentage_vector[i] * log(1.0 / percentage_vector[i])
  }
...
diversity_v <- numeric(nrow(diversity_data))

for (i in 1:nrow(diversity_data)) {
    d <- 
  diversity_data %>%
    slice(i) %>%
    select(4:9) %>%
  ...
  diversity_v[i] <- prod(e)
}


max_d <- max(diversity_v)


diversity_data <-
  diversity_data %>%
  mutate(diversity = diversity_v / max_d * 100 %>% round(1)) 


# 
# diversity_data <-
#   diversity_data %>%
#   mutate(diversity = entropy_index(diversity_data[4:9]))


diversity_county <-
  diversity_data %>%
  filter(!str_detect(County, "Alaska"),  !str_detect(County, "Hawaii")) %>%
  mutate(
    county = as.numeric(GEO.id2), 
    county_state = `GEO.display-label`,
